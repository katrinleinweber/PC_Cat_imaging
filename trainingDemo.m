function [] = trainingDemo(subjectID,order,mainPath,isMRI, use_eyetracker,CueType)
...
Instructions = dir([mainPath '/Instructions/Training_Reward_Between_Demo_cue' num2str(CueType) '.JPG' ]);
...
image=dir('./Stim/demo/*.bmp');
image_read=imread(['./Stim/demo/',image(1).name]);
...
% set cue values for all runs- in order to control the mean of reward for each 'go' item
    ...
    LowReward=21;
    RewardVec=repmat(LowReward:LowReward+VarNum-1,size(vars{1,1},1),floor(numRuns/VarNum)+mod(numRuns,VarNum)*10);
...
for runNum = 1:numRuns
    ...
    r = Shuffle(1:4);
    onsetlist=load(['Onset_files/train_onset_length_80_' num2str(r(1)) '.mat']);
    ...
    %-----------------------------------------------------------------
    % baseline fixation - currently 1 seconds 
    ...
    for trial = 1:length(demoNames) %length(stop{runNum}),   % To cover all the items in one run.
        ...
        CueNum=shuff_rewards(trial,runNum);
        ...
        Screen('Flip',w,onsetlist(trial)+runStartTime); % display images according to Onset times
        ...
        while (GetSecs-image_start_time < image_duration)
            ...
            if  demo_go(trial) == 11 && (GetSecs - image_start_time >= Ladder1{runNum}(length(Ladder1{runNum}),1)/1000) && notone %demo_go contains the information if a certain image is a demo_go trial or not
                ...
                notone = 0;
                Cue_time{runNum}(trial,1) = GetSecs-image_start_time;
                Screen('PutImage',w,demofood_items{trial});
                % Draw the Cue image
        ...
        fprintf(fid1,'%s\t %d\t %s\t %d\t %d\t %d\t %d\t %d\t %d\t %.2f\t %d\t %d\t \n', subjectID, runNum, demoNames{trial}, actual_onset_time{runNum}(trial,1), demo_go(trial), respTime{runNum}(trial,1)*1000, respInTime{runNum}(trial,1), Cue_time{runNum}(trial,1)*1000, keyPressed{runNum}(trial,1),   fix_time{runNum}(trial,1)-anchor, Ladder1{runNum}(length(Ladder1{runNum})), Ladder2{runNum}(length(Ladder2{runNum})));
...
Screen('TextSize', w, TextSize); %Set textsize
