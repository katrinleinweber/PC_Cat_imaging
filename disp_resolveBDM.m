...
% Open data from BDM`
BdmFile=dir([outputPath  subjectID '*_BDM' num2str(sessionNum) '*.txt']);
BDMdata=readtable([outputPath BdmFile(1).name],'delimiter', 'tab');

total_num_items = size(BDMdata,1);
% load images
%! stimuli=dir([pwd '/stim/*.bmp' ]);
%! stimname=struct2cell(rmfield(stimuli,{'date','bytes','isdir','datenum'}));
stimname=BDMdata.Name;
...
DrawFormattedText(Window,'In the auction, the snack chosen is:','center',150,TextColor);
...
while (GetSecs - taim) <= 2,
    ...
    possibleNumbers = Shuffle(0:0.1:3);
...
WaitSecs(1);
%%
fid2 = fopen([outputPath '/' subjectID '_BDM_resolve_session' num2str(sessionNum) '.txt'],'a');
