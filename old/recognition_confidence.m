...
sizeFactor = 0.4 ;
stimW = wWidth*sizeFactor;
stimH = wHeight*sizeFactor;
...
Instructions = dir([mainPath '/Instructions/recognition_confidence.png' ]);
Instructions_fmri = dir([mainPath '/Instructions/recognition_confidence.png' ]);
...
xcenter=wWidth/2;
ycenter=wHeight/2;
InstructionRect= CenterRectOnPointd([ 0 0 wWidth*0.5 wHeight*0.7], xcenter, ycenter);
...
goNoGoQuestion = dir([mainPath '/Instructions/recognition_goNoGo_question.png' ]);
newOldQuestion = dir([mainPath '/Instructions/recognition_oldNew_question.png' ]);
...
question_location = [xcenter-wWidth/3  ycenter-wHeight/2.5  xcenter+wWidth/3 ycenter-wHeight/6   ];
...
recognition_answers = dir([mainPath '/Instructions/recognition_answers.png' ]);
recognition_answers_image = imread([mainPath '/Instructions/' recognition_answers(1).name]);
answer_images = cell(1,5);
for answerInd = 1:5
    answers = dir([mainPath '/Instructions/recognition_' num2str(answerInd) '.png' ]);
    answer_images{answerInd} = imread([mainPath '/Instructions/' answers(1).name]);
end

answers_location=[xcenter-wWidth/3  ycenter+wHeight/3.5 xcenter+wWidth/3 ycenter+wHeight/2.2];
...
newStimName = dir([mainPath '/stim/recognition/*.bmp']); % Read new stimuli
...
for i = 1:length(newStimName)
    imgArraysNew{i} = imread([mainPath '/stim/recognition/' newStimName(i).name],'bmp');
end
...
if test_comp == 1
    Screen('PutImage',w,Instructions_image_fmri,InstructionRect);
    
else
    Screen('PutImage',w,Instructions_image,InstructionRect);
end
Screen(w,'Flip');
...
for trial = 1:length(stimName)
    ...
    Screen('TextSize',w, 60);
    Screen('PutImage',w, newOldQuestion_image,question_location); % display question
    ...
    Screen('PutImage',w, imgArrays{trial},rect); % display item
    Screen('TextSize',w, 60);
    Screen('PutImage',w, goNoGoQuestion_image,question_location); % display question2
    Screen('PutImage',w, recognition_answers_image,answers_location); % display answers
    Screen(w,'Flip');
    ...
    if noresp
        ...
        Screen('PutImage',w,imgArrays{trial}, rect); % display item
        Screen('TextSize',w, 60);
        Screen('PutImage',w, goNoGoQuestion_image,question_location); % display question
