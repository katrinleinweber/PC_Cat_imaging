...
% % % % %  subjectID=101;  order=1; mainPath=pwd; block=1; numRunsPerBlock=1;
% function [trialsPerRun] = organizeProbe_Israel(subjectID, order, mainPath, block, numRunsPerBlock)
%
% = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
...
switch order
    case 1
        ...
        % control comparisons of NOGO items presented in the training 
        % - - - - - - - - - - - - - - 
        controlHV_nobeep = [19 20 21 22]; % HV_nobeep

        controlLV_nobeep = [39 40 41 42]; % LV_nobeep
...
%   TRIAL TYPE 4: HighValue NoGo(Stop) vs. HighValue NoGo(Stop)
% - - - - - - - - - - - - - - - - - - - - - - - - - - -
%since we want to campare the 4 nogo items to themselfs, shuffle them as 2
%seperate vectors, and then remove the pairs that are identical. 

numHVcontrolItems = length(controlHV_nobeep);

HV_control_new = repmat(controlHV_nobeep,numHVcontrolItems,1);
HV_control_new = HV_control_new(:)';


[shuffle_HV_control_new1,shuff_HV_control_new_ind1] = Shuffle(HV_control_new);
[shuffle_HV_control_new2,shuff_HV_control_new_ind2] = Shuffle(HV_control_new);


Items2Delete=shuffle_HV_control_new1==shuffle_HV_control_new2;
shuffle_HV_control_new1(Items2Delete)=[];
shuffle_HV_control_new2(Items2Delete)=[];


    
    
%   TRIAL TYPE 5: LowValue NoGo(Stop) vs. LowValue NoGo(Stop)
% - - - - - - - - - - - - - - - - - - - - - - - - - - -
%since we want to campare the 4 nogo items to themselfs, shuffle them as 2
%seperate vectors, and then remove the pairs that are identical. 

numLVcontrolItems = length(controlLV_nobeep);

LV_control_new = repmat(controlLV_nobeep,numLVcontrolItems,1);
LV_control_new = LV_control_new(:)';


[shuffle_LV_control_new1,shuff_LV_control_new_ind1] = Shuffle(LV_control_new);
[shuffle_LV_control_new2,shuff_LV_control_new_ind2] = Shuffle(LV_control_new);


Items2Delete=shuffle_LV_control_new1==shuffle_LV_control_new2;
shuffle_LV_control_new1(Items2Delete)=[];
shuffle_LV_control_new2(Items2Delete)=[];
...
numControlHV= numHVcontrolItems*(numHVcontrolItems-1)/2; %because it is a comparison of the control items to themselfs
numControlLV= numLVcontrolItems*(numLVcontrolItems-1)/2;
total_num_trials = numComparisons + numSanity +numControlHV + numControlLV;
...
pairType(1:numRunsPerBlock,numComparisonsPerRun+1:numComparisonsPerRun+numSanityPerRun) = 3;
pairType(1:numRunsPerBlock,numComparisonsPerRun+numSanityPerRun+1:numComparisonsPerRun+numSanityPerRun+numControlHV) = 4;
pairType(1:numRunsPerBlock,numComparisonsPerRun+numSanityPerRun+numControlHV+1:numComparisonsPerRun+numSanityPerRun+numControlHV+numControlLV) = 5;
...
controlHV_nobeep1=shuffle_HV_control_new1;
controlHV_nobeep2=shuffle_HV_control_new2;
controlLV_nobeep1=shuffle_LV_control_new1;
controlLV_nobeep2=shuffle_LV_control_new2;
...
HH = 1;
LL = 1;
%HL_S = 1; %sanity for go items- not in use currently
HL_G = 1;
HC=1;
LC=1;
    ...
    for trial = 1:trialsPerRun % trial num within block      
        switch pairType(numRun,trial)
            ...
            case 3
                ...
                % HighValue NoGo(Stop) vs. LowValue NoGo(Stop) - sanity
                % - - - - - - - - - - - - - - - - - - -
            ...
            case 4
                ...
                % HighValue NoGo(Stop) vs. HighValue NoGo(Stop) - control
                % - - - - - - - - - - - - - - - - - - -
                
                stimnum1(numRun,trial) = controlHV_nobeep1(HC);
                stimnum2(numRun,trial) = controlHV_nobeep2(HC);
                HC = HC+1;
                if leftGo(numRun,trial) == 1
                    leftname(numRun,trial) = stimName(stimnum1(numRun,trial));
                    rightname(numRun,trial) = stimName(stimnum2(numRun,trial));
           ...
           case 5
                ...
                % LowValue NoGo(Stop) vs. LowValue NoGo(Stop) - control
                % - - - - - - - - - - - - - - - - - - -
                
                stimnum1(numRun,trial) = controlLV_nobeep1(LC);
                stimnum2(numRun,trial) = controlLV_nobeep2(LC);
                LC = LC+1;
                if leftGo(numRun,trial) == 1
                    leftname(numRun,trial) = stimName(stimnum1(numRun,trial));
                    rightname(numRun,trial) = stimName(stimnum2(numRun,trial));
...
%---------------------------------------------------------------------
outfile = strcat('./Output/', sprintf('%s_stimuliForProbe_order_recall%d_block_%d_%d_trials_%d_runs_%s.mat',subjectID,order,block,total_num_trials,numRunsPerBlock,date));
