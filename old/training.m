...
Cue_time = cell(1,last_run - runInd + 1);
...
% set cue values for all runs- in order to control the mean of reward for each 'go' item
    ...
    LowReward=21;
    RewardVec=repmat(LowReward:LowReward+VarNum-1,size(vars{1,1},1),floor((last_run - runInd + 1)/VarNum)+mod((last_run - runInd + 1) ,VarNum)*10);
...
%for runNum = runInd:last_run %this for loop allows all runs to be completed
    ...
    shuff_rewards(:,runNum)=AllRewards(shuff_ind{runNum},runNum);
     ...
     for trialNum = 1:length(shuff_trialType{runNum})   % To cover all the items in one run.
        ...
        while (GetSecs-image_start_time < image_duration)
                ...
                if isMRI == 1
                    if keyPressed{runNum}(trialNum,1) == blue || keyPressed{runNum}(trialNum,1) == yellow
                        ...
                        if respTime{runNum}(trialNum,1) < Ladder1{runNum}(length(Ladder1{runNum}),1)/1000
                            respInTime{runNum}(trialNum,1) = 11; %was a GO1 trial but responded before SS
                        else
                            respInTime{runNum}(trialNum,1)= 110; %was a Go1 trial but responded after SS within 1000 msec
                        end
                    end
                else
                    if keyPressed{runNum}(trialNum,1) == BUTTON %| keyPressed{runnum}(trialnum,1)==RIGHT
                        ...
                        if respTime{runNum}(trialNum,1) < Ladder1{runNum}(length(Ladder1{runNum}),1)/1000
                            ...
                            respInTime{runNum}(trialNum,1) = 110; %was a Go1 and responded after SS within 1000 msec - good trial
                        end
                    end
                end % if isMRI == 1
            ...
            elseif  shuff_trialType{runNum}(trialNum) == 22 && (GetSecs - image_start_time >= Ladder2{runNum}(length(Ladder2{runNum}),1)/1000) && notone %shuff_trialType contains the information if a certain image is a GO/NOGO trial
                ...
                CueStrComb=['+' num2str(shuff_rewards(trialNum,runNum))];
                notone = 0;
                Cue_time{runNum}(trialNum,1) = GetSecs-image_start_time;
                Screen('PutImage',w,food_items{trialNum});
                % Draw the Cue image
                ...
                if isMRI == 1
                    if keyPressed{runNum}(trialNum,1) == blue || keyPressed{runNum}(trialNum,1) == yellow
                        ...
                        if respTime{runNum}(trialNum,1) < Ladder2{runNum}(length(Ladder2{runNum}),1)/1000
                            respInTime{runNum}(trialNum,1) = 22; %was a GO2 trial but responded before SS
                        else
                            respInTime{runNum}(trialNum,1) = 220; %was a Go2 trial but responded after SS within 1000 msec
                        end
                    end
                else
                    if keyPressed{runNum}(trialNum,1) == BUTTON %| keyPressed{runnum}(trialnum,1)==RIGHT
                ...
                %No-BEEP
                %---------------------------
            elseif   shuff_trialType{runNum}(trialNum) == 33 && noresp % these will now be the NOGO trials
                % different response types in scanner or in testing room
                if isMRI == 1
                    if keyPressed{runNum}(trialNum,1) == blue || keyPressed{runNum}(trialNum,1) == yellow
                        noresp = 0;
                        respInTime{runNum}(trialNum,1) = 33; % a stop trial but responded within 1000 msec HV item - not good but don't do anything
                    end
                else
                    if keyPressed{runNum}(trialNum,1) == BUTTON %| keyPressed{runnum}(trialnum,1)==RIGHT
                        noresp = 0;
                        respInTime{runNum}(trialNum,1) = 33; %% a stop trial but responded within 1000 msec LV item - not good but don't do anything
                    end
                    ...
                    if isMRI == 1
                        if keyPressed{runNum}(trialNum,1) == blue || keyPressed{runNum}(trialNum,1) == yellow
                            ...
                            switch shuff_trialType{runNum}(trialNum)
                                ...
                                case 33
                                    respInTime{runNum}(trialNum,1) = 33; % a stop trial and responded after 1000 msec  HV item - don't touch
                        ...
                        if keyPressed{runNum}(trialNum,1) == BUTTON % | keyPressed{runnum}(trialnum,1)==RIGHT
                            ...
                            switch shuff_trialType{runNum}(trialNum)
                                ...
                                case 33
                                    respInTime{runNum}(trialNum,1) = 33;% a NOGO trial and didnt respond on time HV item - don't touch
        ...
        if noresp
            switch shuff_trialType{runNum}(trialNum)
                ...
                case 33
                    respInTime{runNum}(trialNum,1) = 330; % ok NOGO trial didn't respond after 1500 msec in NOGO trial HV
            end
        end
        
        
        switch respInTime{runNum}(trialNum,1)
            case 1 % didn't respond even after 1500 msec on GO1 trial - make it easier decrease SSD by step
            ...
            case 2 % didn't respond even after 1500 msec on GO2 trial - make it easier decrease SSD by step
            ...
            case 1100 %  responded after 1500 msec on GO1 trial - make it easier decrease SSD by step
                if (Ladder1{runNum}(length(Ladder1{runNum}),1)<0.001)
                    Ladder1{runNum}(length(Ladder1{runNum})+1,1) = Ladder1{runNum}(length(Ladder1{runNum}),1);
                else
                    Ladder1{runNum}(length(Ladder1{runNum})+1,1) = Ladder1{runNum}(length(Ladder1{runNum}),1)-Step;
            ...
            case 2200 %  responded after 1500 msec on GO2 trial - make it easier decrease SSD by step
                if (Ladder2{runNum}(length(Ladder2{runNum}),1)<0.001)
                    Ladder2{runNum}(length(Ladder2{runNum})+1,1) = Ladder2{runNum}(length(Ladder2{runNum}),1);
                else
                    Ladder2{runNum}(length(Ladder2{runNum})+1,1) = Ladder2{runNum}(length(Ladder2{runNum}),1)-Step;
            ...
            case 11
                if (Ladder1{runNum}(length(Ladder1{runNum}),1) > 910); %was a GO1 trial but responded before SS make it harder - increase SSD by Step/3
                    Ladder1{runNum}(length(Ladder1{runNum})+1,1) = Ladder1{runNum}(length(Ladder1{runNum}),1);
                else
            ...
            case 22
                if (Ladder2{runNum}(length(Ladder2{runNum}),1) > 910); %was a GO2 trial  but responded before SS make it harder - - increase SSD by Step/3
                    Ladder2{runNum}(length(Ladder2{runNum})+1,1) = Ladder2{runNum}(length(Ladder2{runNum}),1);
                else
                    Ladder2{runNum}(length(Ladder2{runNum})+1,1) = Ladder2{runNum}(length(Ladder2{runNum}),1)+Step/3;
                end;
                
            case 110 % pressed after Go1 signal but below 1000 - - increase SSD by Step/3 - these are the good trials!
            ...
            case 220 % pressed after Go2 signal but below 1000 - - increase SSD by Step/3 - these are the good trials!
