...
%set text size of the dialod box
set(groot,'defaultUicontrolFontSize', 16)
DontEat = myinputdlg('Are there certain foods you do not eat? Please specify ','DontEat',1);
DontEat = cell2mat(DontEat);
...
fprintf(fid1,'%s\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t%s\n', subjectID, order, timestamp, Gender, Age, DominantHand,Height, Weight, Occupation, Eat, DontEat, Hungry, Comments);
fclose(fid1);
end
